...
def build_medical_prompt(
    ...
    """Builds a structured prompt instructing the LLM to output grounded JSON evidence."""
    source_summaries: list[str] = []

    for index, source in enumerate(sources, start=1):
        trial_data = source.get("trial") or {}
        trial_status = trial_data.get("status") if isinstance(trial_data, dict) else "N/A"
        ...
        source_summaries.append(
            ...
            f"Type: {source.get('type', 'publication')}\n"
            f"YEAR: {source.get('year') or 'Unknown'}\n"
            f"Summary: {summary}\n"
            f"Trial Status: {trial_status or 'N/A'}"
        )
...
Context:
- Disease: {context.get('disease') or 'Unknown'}
- Intent: {context.get('intent') or 'Unknown'}
- Location: {context.get('location') or 'Unknown'}
- Follow-up: {"Yes" if context.get('isFollowUp') else "No"}
